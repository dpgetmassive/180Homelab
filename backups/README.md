...
│      └─> Stored on pve-bk-truenas-primary (NFS from Primary TrueNAS)
...
### Storage Capacity
- **pve-bk-truenas-primary Total**: 7.4 TB available (NFS from Primary TrueNAS)
- **Current Usage**: 0 GB (0%)
- **Retention**: keep-last=7, keep-monthly=1 (automated prune)
- **Growth Rate**: ~10-15 GB/day
- **Estimated Capacity**: ~740 days before full (with current growth rate)
...
### 2025-11-20: Storage Configuration Fix (v2.3)
- **Fixed storage verification**: Changed from SSH-based remote check to local verification
- **Corrected NFS mount**: Updated from pve-itchy local storage to NFS mount from Primary TrueNAS
- **Added write test**: Verifies storage is writable before starting backup operations
- **Successful test**: Full backup of 19 VMs/CTs completed in 36 minutes
- **Storage path**: Now using `/mnt/pve/pve-bk-truenas-primary` (7.4TB NFS from 10.16.1.6)

### Key Changes in Script:
```bash
# Lines 61-75: Fixed storage verification
if pvesm status | grep -q "pve-bk-truenas-primary.*active"; then
    if touch /mnt/pve/pve-bk-truenas-primary/.test 2>/dev/null; then
        rm -f /mnt/pve/pve-bk-truenas-primary/.test
        log "✓ pve-bk-truenas-primary storage is mounted and writable"
```

This resolves the storage check failures and ensures backups write to the correct NFS location with proper redundancy.
...
**Script Version**: 2.3 (Fixed storage verification and NFS configuration)
